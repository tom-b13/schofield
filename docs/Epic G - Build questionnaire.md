...
# 7.4.2 Integration Tests - Step Function Definitions

7.4.2.1) Given screen "<screen_id>" exists with questions:

Pattern (regex/params)

Step: ^screen "([^"]+)" exists with questions:$

Params:

screen_id (string)

Data table with columns: question_id (string), question_text (string), question_order (integer)

Purpose
Seed a screen and its ordered questions so reads return a deterministic list.

Implementation outline (backend, boundary-first)

Create or resolve a questionnaire context as per test harness policy.

Call authoring API to create the screen if absent; otherwise retrieve it.

For each table row, create or upsert the question on this screen with the given text and proposed order; final order is authoritative server-side.

Re-fetch the screen’s questions to confirm the stored order matches the table’s question_order sequence; store fixture handles for later steps.

State carried forward

Save screen_id and the created questions in ctx.seed.screen and ctx.seed.questions.

7.4.2.2) When I GET "<path>"
...
path (string; absolute path only, e.g. /screens/S-2/questions)

Purpose
Invoke the read endpoint under test and capture response.

Implementation outline

Issue HTTP GET to <path> against the test server.

Store response status, headers (normalised to lowercase keys), and parsed JSON body into ctx.response.

State carried forward

ctx.response.status, ctx.response.headers, ctx.response.body.

7.4.2.3) Then the response status should be <status>

Pattern (regex/params)

Step: ^the response status should be (\d+)$
...
Compare ctx.response.status with status (exact match).

On mismatch, include last response body snippet for diagnostics.

7.4.2.4) And the response should include headers "<header_names>"

Pattern (regex/params)

Step: ^the response should include headers "([^"]+)"$

Params:

header_names (string; comma-separated or single name)
...
Split header_names by comma, trim, lower-case each.

For each, assert key exists in ctx.response.headers.

Do not assert header values here (presence only).

7.4.2.5) And the response body should include:

Pattern (table)

Step: ^the response body should include:$

Data table columns:

path (string; dot/bracket notation, e.g. questions[0].question_id)

value (string literal; treat numeric strings as numbers when unquoted in body)

Purpose
Assert selected JSON fields and ordering.

Implementation outline

For each row, navigate ctx.response.body using the path (supports array indices [...] and object keys via dots).

Coerce expected value if it’s a pure integer (e.g. 1) to numeric comparison; otherwise compare as string.

Assert equality for each path/value pair.

Fail with clear diff showing actual extracted value.
